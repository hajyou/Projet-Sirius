...
while boucles1 == True:
    ...
    print("\n-------------- Version 1.0 --------------")
    print("[SYSTEM Y.H ] : Programme (Dépenses) lancé !")
    ...
    while boucles == True:
        ...
        new_values = (time.strftime("%d/%m/%Y"), input("\nEntrer votre désignation \n>>"), float(input("\nEntrer votre dépense \n>>")))

        cursor.execute('INSERT INTO tt_donnees VALUES(?, ?, ?)', new_values)
        connection.commit()
        req2 = cursor.execute('SELECT * FROM tt_donnees')
        ...
        if demande == "o":
            ...
            while 1:    
                if verif == 'o':
                    req = 'DELETE FROM tt_donnees'
                    cursor.execute(req)
                    connection.commit()
                    time.sleep(1)
                    print("Votre base de données a bien été nettoyer .. \n")
                    print("\n_____________________ Base de données ___________________________\n")
                    print("\n____________  Vide (Tapez Ctrl + z pour quitter !)  _____________\n")
                    time.sleep(2)
                    break
                    while 1:   
